% =============================================================================
% Reasoning Patterns — How Do I Ground and Reason?
% Purpose: CoT, self-consistency, ReAct, ToT/GoT, few-shot, reflection
% Label: sec:llmB-reason
% =============================================================================

\section{Grounded Reasoning Patterns}
\label{sec:llmB-reason}

The previous sections established how to ground model responses in authoritative sources through retrieval, domain-specific filtering, and matter isolation. With grounding mechanisms in place, we now turn to eliciting structured reasoning \emph{over} that grounded context.

The distinction matters: reasoning without grounding is speculation; grounding without reasoning is mere retrieval. Professional applications require both. An attorney needs the model to find the relevant cases (grounding) \emph{and} apply them systematically to the client's situation (reasoning). A financial analyst needs the model to retrieve the relevant disclosures (grounding) \emph{and} synthesize them into actionable analysis (reasoning).

This section covers reasoning patterns that become dramatically more powerful when applied to grounded context. We start with relatively lightweight techniques and progress to more elaborate ones, discussing when each is appropriate. The guiding principle is: \emph{use the simplest strategy that gets the job done reliably}. Overly complex prompting can waste time or even confuse the model if the task didn't need it. On the other hand, a task that requires multi-step reasoning over retrieved evidence will benefit greatly from these patterns.
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.9\textwidth}{!}{\input{chapters/03-reasoning-conversations/figures/fig-reasoning-patterns}}
...
\paragraph{Example: Legal Analysis with Grounded Context.} Consider a question about contract liability where the model has been provided with the actual contract and relevant case law via RAG:

\begin{quote}
\textbf{Direct prompting}: ``Is the seller liable for the defective goods under this contract?'' \\
\textbf{Model response}: ``Yes.'' (potentially incorrect, no reasoning visible, no citation)
\end{quote}

\begin{quote}
\textbf{Chain-of-thought prompting over grounded context}: ``Based on the retrieved contract (Exhibit A) and the Delaware case law provided, analyze the contract liability step by step. First, identify the relevant warranty provisions in the contract. Second, apply the standard from \textit{Coastal Steel Corp. v. Tilghman Wheelabrator} to determine whether the defect was discoverable at delivery. Third, assess whether proper notice was given under Section 8.3. Finally, conclude on liability with citations.'' \\
\textbf{Model response}: ``Step 1: The contract contains an express warranty in Section 4.2 stating `Seller warrants that all goods shall be free from defects in material and workmanship.' [Exhibit A, p.~7] Step 2: Under \textit{Coastal Steel}, a defect is latent if it `could not have been discovered by reasonable inspection.' The heating element failure occurred after 6 months of normal use, indicating a manufacturing defect not discoverable at delivery. Step 3: Notice was provided by email on January 15, 2024, within the 30-day period required by Section 8.3. Conclusion: Based on the express warranty at Section 4.2 and the latent nature of the defect under Delaware law, the seller is likely liable.''
\end{quote}

The grounded chain-of-thought version forces the model to (1) reason over specific retrieved documents rather than general knowledge, and (2) cite sources at each step. The answer is now verifiable.
...
\begin{figure}[htbp]
  \centering
  \resizebox{0.75\textwidth}{!}{\input{chapters/03-reasoning-conversations/figures/fig-react-loop}}
...
\begin{highlightbox}[title={ReAct Pattern}]
The ReAct pattern---reasoning interleaved with action---represents a proto-agentic architecture. Our companion volume, \textit{Agentic AI in Law and Finance}, formalizes when such systems qualify as genuine agents and how to architect them, providing a rigorous framework distinguishing agents from tool-using systems and addressing how agents decompose tasks and execute tool calls.
\end{highlightbox}
...
These exploration patterns inform agent planning. Our companion volume, \textit{Agentic AI in Law and Finance}, addresses how agents break complex jobs into steps, manage dependencies, and backtrack when needed---operationalizing ToT/GoT as planning strategies.
...
\begin{highlightbox}[title={Self-Reflection Example}]
...
In agentic contexts, self-consistency informs termination conditions---how agents know when they've reached a reliable answer. Our companion volume addresses explicit stopping conditions, goal satisfaction verification, and resource budgets for agentic systems.
...
\subsubsection{Dynamic Example Selection}
\label{sec:llmB-dynamic-selection}

Selecting examples that are relevant to the user's query is crucial. Research shows that retrieving in-context examples that closely match the query's topic or wording significantly improves performance over random selection \parencite{zebaze2024icl}.

\paragraph{Forward Reference.} Chapter~7 (Prompt Design, Evaluation, and Optimization) provides comprehensive coverage of dynamic exemplar retrieval, including:
\begin{itemize}
  \item \textbf{K-Nearest Neighbors (K-NN)}: Retrieving the $k$ most semantically similar examples
  \item \textbf{Maximal Marginal Relevance (MMR)}: Balancing relevance and diversity in selection
  \item \textbf{Bootstrapping}: Using the model itself to generate synthetic exemplars when examples are scarce
  \item \textbf{Exemplar library management}: Organization, versioning, and staleness prevention
\end{itemize}

For reasoning tasks, the key insight is that example selection directly affects reasoning quality. Domain-matched examples (M\&A examples for M\&A questions) outperform generic examples, even when the logical task structure is similar.
...
\begin{keybox}[title={Pattern Selection}]
\begin{itemize}
