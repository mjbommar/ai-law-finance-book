...
\begin{keybox}[title={Strategy Selection}]
\begin{itemize}
...
\small
\begin{tabular}{@{}p{1.8cm}p{1.9cm}p{1.4cm}p{1.4cm}p{3.0cm}@{}}
\toprule
\textbf{Strategy} & \textbf{Reasoning} & \textbf{Latency} & \textbf{Cost} & \textbf{Use Case} \\
\midrule
Direct & Implicit & Low & Low & Simple, low-risk tasks \\
\addlinespace
CoT & Explicit (1) & Medium & Medium & Multi-step reasoning \\
\addlinespace
Self-Consist. & Explicit (N) & High & High & Critical decisions \\
\addlinespace
ReAct & Explicit+Tools & Med-High & Med-High & Tool workflows \\
\addlinespace
ToT/GoT & Branching & Very High & Very High & Exploration, planning \\
\addlinespace
Modular & Per-module & Medium & Medium & Production systems \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Reasoning Strategies: Reference to Chapter~3}
\label{sec:llmE-reasoning-ref}

This section provides a brief summary of reasoning strategies for quick reference. For comprehensive treatment including theoretical foundations, implementation details, and domain-specific adaptations, see Section~\ref{sec:llmB-reason} (Chapter~3: Reasoning and Conversations).
...
\begin{keybox}[title={Modular Architecture}]
...
\begin{highlightbox}[title={Delegation Pattern}]
...
\begin{keybox}[title={Start Simple}]
