...
\begin{definitionbox}[title={Phase 4: Structured I/O}]
\textbf{Characteristics}:
\begin{itemize}
...
\begin{keybox}[title={Production Minimum}]
...
\begin{highlightbox}[title={Agentic Architectures}]
...
\small
\begin{tabular}{@{}p{1.6cm}p{1.8cm}p{1.8cm}p{1.6cm}p{2.8cm}@{}}
\toprule
\textbf{Phase} & \textbf{Input} & \textbf{Output} & \textbf{Testability} & \textbf{Use Case} \\
\midrule
1. Zero-Shot & Unstructured & Freeform text & Very Low & Exploration, prototyping \\
\addlinespace
2. Few-Shot & Text + examples & Guided text & Low & Format teaching \\
\addlinespace
3. Struct. Input & JSON/XML & Freeform text & Medium & Defined input analysis \\
\addlinespace
4. Struct. I/O & JSON/XML & JSON/XML & High & \textbf{Production min.} \\
\addlinespace
5. Modular & JSON/XML & JSON/XML & Very High & Mission-critical \\
\bottomrule
\end{tabular}
\end{table}
...
\small
\begin{tabular}{@{}p{3.0cm}p{3.2cm}p{4.5cm}@{}}
\toprule
\textbf{Use Case} & \textbf{Recommended} & \textbf{Rationale} \\
\midrule
Task instructions & Markdown & Human-readable, hierarchical \\
\addlinespace
Input data & JSON & Compact, machine-parseable \\
\addlinespace
Output schema & JSON & Validation, app integration \\
\addlinespace
Legacy integration & XML & Compatibility with standards \\
\addlinespace
Legal doc markup & XML (if mandated) & Explicit structure, validation \\
\addlinespace
Few-shot examples & Markdown + JSON & Markdown for text, JSON for data \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{keybox}[title={Exemplar Design}]
\begin{enumerate}
...
\paragraph{K-Nearest Neighbors (K-NN) Selection.} The simplest approach retrieves the $k$ exemplars whose embeddings are most similar to the current input. This ensures topical relevance but may produce redundant examples.

\paragraph{Maximal Marginal Relevance (MMR).} To balance relevance and diversity, \keyterm{MMR} iteratively selects exemplars that are both similar to the query and dissimilar to already-selected examples:
\[
\text{MMR} = \lambda \cdot \text{Sim}(d, q) - (1 - \lambda) \cdot \max_{d_j \in S} \text{Sim}(d, d_j)
\]
where $d$ is the candidate, $q$ is the query, $S$ is the selected set, and $\lambda$ (typically 0.5) balances the two objectives.
...
\begin{enumerate}
  ...
  \item \textbf{Retrieve with K-NN or MMR}: Use cosine similarity or vector search (FAISS, Pinecone)
...
\begin{enumerate}
  \item Use a capable model to generate candidate reasoning chains for a set of questions
  \item Filter candidates: keep only those leading to verified correct answers
  \item Use these correct, synthetic chains as few-shot exemplars
\end{enumerate}

This technique (sometimes called ``Self-Instruct'') allows rapid creation of high-quality exemplar libraries without extensive manual annotation \parencite{wang2023selfinstruct}.

\begin{cautionbox}[title={Bootstrapping Cautions}]
\begin{itemize}
  ...
  \item \textbf{Track provenance}: Note which exemplars are human-curated vs.\ AI-generated
  \item \textbf{Domain expertise}: For legal, medical, or financial domains, have an expert vet synthetic examples
\end{itemize}
\end{cautionbox}
...
\clearpage% Start checklist on fresh page to prevent bottom overflow

\begin{keybox}[title={Design Checklist}]
...
\begin{keybox}[title={Operations Checklist}]
