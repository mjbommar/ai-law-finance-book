...
\begin{definitionbox}[title={Three Control Layers}]
\begin{enumerate}
...
\noindent Chapter~7 (Prompt Design, Evaluation, and Optimization) addresses schema versioning and migration strategies in detail, including strategies for A/B testing schema changes and rolling back incompatible updates.
...
\noindent In practice, many systems start with prompt schemas during development and migrate to function calling once the schema stabilizes. Chapter~7 (Prompt Design, Evaluation, and Optimization) discusses evaluation strategies for validating schema changes before deployment.

\begin{highlightbox}[title={Function Calling: Two Purposes}]
Function calling serves two distinct purposes in LLM systems:
\begin{itemize}
  \item \textbf{Structured output} (this section): Using function calling as a mechanism to guarantee the LLM produces valid JSON matching a schema---no external tools are actually invoked.
  \item \textbf{External tool invocation} (\Cref{sec:llmC-tools}): Using function calling to invoke external systems (calculators, databases, APIs) and incorporate their results.
\end{itemize}
Both use the same API mechanisms but serve different architectural purposes. Structured output hardens the LLM's responses; tool use extends the LLM's capabilities.
\end{highlightbox}
...
\noindent Chapter~7 (Prompt Design, Evaluation, and Optimization) covers schema versioning in the context of prompt evaluation and A/B testing, including strategies for validating schema changes before rollout.
...
\begin{itemize}
  \item \textbf{Chapter~7 (Prompt Design, Evaluation, and Optimization)}:
  Discusses schema versioning, A/B testing schema changes, and evaluating classification accuracy.

  \item \textbf{Agentic AI in Law and Finance} (companion volume):
  Addresses how schema design enables agentic auditability and transparency---structured outputs become the audit trail for automated decisions.
\end{itemize}
