% =============================================================================
% Grounding & Retrieval (RAG Fundamentals) — How Do I Ground and Reason?
% Purpose: Source-grounding, chunking, embeddings, indexing, freshness, citations
% Label: sec:llmB-rag
% =============================================================================

\section{Connecting to Your Sources: Retrieval-Augmented Generation}
\label{sec:llmB-rag}

The previous section established that in-context learning enables models to adapt based on what appears in their context window. This section examines the practical technique for populating that context with relevant information: \keyterm{Retrieval-Augmented Generation} (RAG).

RAG is a pattern where the model is provided with relevant documents or data from an external source---a knowledge base, database, or search index---to use when generating its answer. Instead of relying solely on what is stored in the model's parameters (which could be outdated or insufficiently detailed), the model augments its answer with retrieved information. For legal and financial professionals, this means an AI can cite the actual law, regulation text, or financial report that supports its conclusion, rather than giving an answer you have to take on faith.

Embeddings provide the similarity signal that powers retrieval; see the primer in Chapter~1, Section~\ref{sec:llm1-embeddings}. All retrieval outputs should carry the canonical evidence record defined in Chapter~4 (\Cref{sec:llmC-evidence}).

% -----------------------------------------------------------------------------
\subsection{Why Grounding Matters}
\label{sec:llmB-rag-why}
% -----------------------------------------------------------------------------
...
\begin{cautionbox}[title={Hallucination Risk Remains}]
Retrieval does not eliminate hallucination. A preregistered study of commercial legal AI tools found that RAG-enabled systems hallucinate in 17--33\% of responses even when relevant documents are retrieved \parencite{magesh2025hallucinations}. Verification mechanisms (Chapter~3: Evidence Records) remain essential.
\end{cautionbox}

The next section examines professional domain requirements that generic retrieval systems do not provide: authority hierarchies, jurisdictional filtering, temporal validity, and matter isolation. These constraints distinguish professional AI systems from consumer chatbots.
