...
\begin{itemize}
  \item \textbf{Chapter 2 (Conversations and Reasoning)} adds multi-turn dialogue, chain-of-thought prompting, and reasoning strategies. Understanding tokens and context windows here is prerequisite for managing conversation state there.

  \item \textbf{Chapter 3 (Retrieval-Augmented Generation)} expands the embedding concepts introduced here into full RAG architectures. The discussion of hybrid search and vector databases here provides the conceptual foundation.

  \item \textbf{Part II (Agents)} assumes familiarity with all concepts in this chapter. Tool-calling agents require structured outputs (constrained decoding), and agent loops must handle the stochastic nature of LLM outputs.

  \item \textbf{Governance Chapters} build on the failure mode taxonomy here to specify validation frameworks, monitoring systems, and compliance controls.
\end{itemize}
