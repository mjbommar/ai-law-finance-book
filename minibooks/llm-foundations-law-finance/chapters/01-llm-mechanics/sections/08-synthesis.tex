...
\begin{keybox}[title={Control Stochasticity}]
...
\begin{keybox}[title={Human Oversight}]
...
\toprule
\textbf{Requirement} & \textbf{Relevant Mechanics} & \textbf{Implementation} \\
\midrule
Auditability & Sampling determinism, logging & Low temp, seed, full logging \\
Accuracy & Hallucination mitigation & RAG, citations, verification \\
Currency & Knowledge cutoff & Retrieval of current docs \\
Reproducibility & Sampling parameters & Fixed seeds, versioned prompts \\
Security & Prompt injection & Input sanitization, sandboxing \\
Privacy & Training data, embeddings & On-prem deployment, governance \\
Explainability & Output structure & Structured outputs, citations \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Chapter 2 (Conversations and Reasoning).} Extends single-turn mechanics to multi-turn dialogue, introducing conversation state management, chain-of-thought and structured reasoning strategies, role-based prompting, and context window management across turns.

\paragraph{Chapter 3 (Structured Outputs and Tool Use).} Moves from freeform text to production-grade systems with JSON/XML schemas, tool calling and function execution, retrieval-augmented generation for grounding, and audit trails for compliance.

\paragraph{Chapter 4 (Multimodal Inputs).} Extends LLM capabilities beyond text to handle the documents professionals actually work with: PDFs with complex layouts, tables and financial statements, charts and visual data, and audio transcriptions. The same principles of structured extraction and evidence records apply.

\paragraph{Chapter 5 (Prompt Design, Evaluation, and Optimization).} Treats prompting as engineering discipline with systematic design patterns, empirical evaluation and metrics, version control and regression testing, and threat modeling for adversarial robustness.

\paragraph{Agentic Systems.} Our companion volume, \textit{Agentic AI in Law and Finance}, introduces LLMs as components in autonomous systems with planning, execution, and governance frameworks.
