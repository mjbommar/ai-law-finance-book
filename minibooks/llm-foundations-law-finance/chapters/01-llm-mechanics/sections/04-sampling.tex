...
\section{Sampling and Decoding: Controlling Generation}
\label{sec:llm1-sampling}
...
\toprule
\textbf{Temperature} & \textbf{Use Case} & \textbf{Rationale} \\
\midrule
...
\begin{keybox}[title={Recommended Defaults for Regulated Applications}]
\begin{itemize}
...
\paragraph{Model Updates.} Cloud-hosted models are updated periodically. What you call ``GPT-4'' today may be a subtly different model than what you called ``GPT-4'' last month. Behavior can change even with identical prompts.
...
\begin{enumerate}
  ...
  \item \textbf{Document model version.} Note the specific model identifier, not just ``GPT-4'' but ``gpt-4-0613'' or equivalent.
...
\paragraph{Where to go deeper.} Chapter~2 (Conversations and Reasoning) provides the primary treatment of self-consistency as a reasoning reliability strategy, including when to use it (and when not to) in professional workflows.
...
\toprule
\textbf{Application} & \textbf{Temp} & \textbf{Top-p} & \textbf{Max Tokens} & \textbf{Notes} \\
\midrule
