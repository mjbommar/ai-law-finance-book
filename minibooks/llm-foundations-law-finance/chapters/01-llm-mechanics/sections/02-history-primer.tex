...
\paragraph{Tool Use.} Modern LLMs can be configured to use external tools---calculators, code interpreters, databases, web search---by generating structured ``tool calls'' that are executed by surrounding systems \parencite{lewis2020rag}. This is not built into the LLM itself but achieved through:
\begin{itemize}
  \item Training to recognize when external information is needed
  \item Outputting structured requests (typically JSON) for tool invocation
  \item Incorporating tool outputs into subsequent responses
\end{itemize}
...
\paragraph{Multimodal Models.} Some advanced models (GPT-4o, Claude, Gemini) can process images, PDFs, and other modalities alongside text. This enables:
\begin{itemize}
  \item Analyzing scanned documents without OCR pre-processing
  \item Reviewing charts and figures in financial reports
  \item Processing handwritten notes or annotations
\end{itemize}
