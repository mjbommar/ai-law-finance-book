...
\end{itemize}
Reasoning traces make decisions transparent and auditable. ReAct works well for exploratory tasks where you learn as you go, such as legal research, fact investigation, and market analysis.

\input{chapters/02-how-to-design/figures/fig-react-cycle}
...
\end{itemize}
Each specialist may decompose further. This enables parallelization and specialization, mirroring how litigation teams work. \Cref{sec:agents2-delegation} details these delegation patterns. \Cref{fig:agents2-hierarchical-planning} illustrates this decomposition.

\input{chapters/02-how-to-design/figures/fig-hierarchical-planning}
...
\glsadd{tool-orchestration}	\textbf{Static orchestration} executes predefined workflow graphs. The same input always produces the same execution path. It is predictable and auditable but inflexible.
...
\begin{table}[htbp]
	...
	\begin{tabular}{
		...
		\toprule
		\textbf{Task Type}              & \textbf{Pattern} & \textbf{Autonomy} \\
		\midrule
		Well-defined steps, known scope & Plan-Execute     & Moderate          \\
		\midrule
		Exploratory, learns as it goes  & ReAct            & Higher            \\
		\midrule
		Complex, parallel workstreams   & Hierarchical     & Distributed       \\
		\bottomrule
	\end{tabular}
...
The mapping is intuitive once you see examples. Plan-Execute suits well-defined procedures: credit agreement reviews, compliance audits, due diligence checklists. ReAct fits exploratory work where the agent learns as it goes: legal research, fact investigation, market analysis. Hierarchical patterns handle complex engagements with parallel workstreams: M\&A transactions, portfolio construction, multi-jurisdiction filings. Higher autonomy requires more sophisticated oversight, and each pattern demands a different approach.
...
Task classification shapes the choice of planning pattern. An exploratory question like ``What are the key risks in this contract?'' calls for the flexible, iterative approach of ReAct. A well-defined multi-step procedure, such as ``Review this document against our standard checklist,'' fits the structured decomposition of Plan-Execute. Complex engagements with nested subtasks and dependencies require hierarchical planning with explicit coordination. Choosing the wrong pattern wastes resources or produces inadequate results.
