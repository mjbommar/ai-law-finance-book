...
Servers expose three types of primitives. \keyterm{Resources} provide read-only data access---document repositories, market data feeds, regulatory databases. The read-only designation matters: an agent with resource access can retrieve documents but cannot modify them. \keyterm{Tools} are executable functions that can change state---filing documents, sending messages, executing queries. \keyterm{Prompts} are reusable templates encoding standard procedures---checklists, SOPs, and structured workflows that ensure consistency.

For perception specifically, Resources are the primary primitive. An agent perceives external information by querying Resources exposed by MCP Servers. The separation between Resources (read-only) and Tools (read-write) enables fine-grained access control that mirrors how organizations already manage permissions.
