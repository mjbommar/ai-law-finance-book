...
\begin{definitionbox}[title={Instruction}, breakable=false]
...
LLM-based agentic systems replace explicit routing logic with semantic reasoning. Instead of maintaining explicit rules for every event type, the model \textit{understands} what kind of work this is. ``Review this credit agreement'' and ``Check this loan doc for problems'' express similar intents despite different words; the LLM recognizes both as document review tasks without explicit rules mapping each phrase to a handler.
...
Production systems often combine both. Simple, high-volume, time-sensitive tasks use deterministic rules, so a margin call always routes to the trading desk. Complex, ambiguous, or novel requests use LLM reasoning. The rule-based layer handles predictable cases efficiently while the LLM layer handles everything else.
...
\begin{cautionbox}[title={Intent Inference Is Not Mind Reading}]
	LLMs infer \textit{probable} intent from language patterns; they do not read minds. Design for clarification, not guessing.
\end{cautionbox}

Inference fails when instructions are genuinely ambiguous, when the user's intent differs from typical patterns, when critical context exists outside the conversation (prior meetings, firm norms), or when the user themselves is unclear about what they want. The aim is an agent that surfaces uncertainty and asks, not one that pretends certainty regarding intent.
...
\begin{tikzpicture}[
	...
	\node[inbox] (input) at (-2.0, 0) {
		\textcolor{gray-500}{\tiny\sffamily\bfseries USER INSTRUCTION}\\[4pt]
		\small\sffamily\textit{``Review the Smith}\\
		\small\sffamily\textit{acquisition agreement}\\
		\small\sffamily\textit{for change-of-control}\\
		\small\sffamily\textit{provisions''}
	};

	% Arrow
	\draw[arrow] (input.east) -- ++(0.5, 0);

	% Right: Extracted goal
	\node[goalbox, right=0.8cm of input] (goal) {
		\textcolor{example-dark}{\small\sffamily\bfseries Structured Goal}\\[3pt]
		\scriptsize\sffamily
		\begin{tabular}{@{}l@{\hspace{0.4em}}l@{}}
			...
			\textcolor{gray-600}{Objective:}   & Identify CoC provisions            \\
			\textcolor{gray-600}{Deadline:}    & January 15, 2025                   \\
			\textcolor{gray-600}{Scope:}       & Sections 5--8                      \\
			\textcolor{gray-600}{Deliverable:} & Summary memo                       \\
			\textcolor{gray-600}{Success:}     & All provisions identified          \\
		\end{tabular}
	};

\end{tikzpicture}
\caption{Goal extraction transforms natural language instructions into structured specifications that guide agent execution and define success criteria.}
\label{fig:agents2-goal-extraction}
\end{figure}
...
\input{chapters/02-how-to-design/figures/fig-when-to-clarify}
...
\begin{quote}
	\textbf{Poor:} ``Can you clarify?''

	\textbf{Better:} ``Should I assess lender risks, borrower risks, or both?''

	\textbf{Best:} ``You asked to reduce tech exposure. Should I (a) sell tech to 25\% target, (b) hedge with options, or (c) add non-tech positions? Which deadline matters: this week or month-end reporting?''
\end{quote}

Research has documented that LLMs sometimes select a default interpretation instead of asking for clarification, even when ambiguity is significant \parencite{zhang2024clarifying,wang2024askwhenneed}. This ``proceed without asking'' behavior creates real risk: the agent interprets ``review the contract'' as a surface-level summary when the user expected deep issue-spotting, delivering work product that is technically responsive but wrong.
...
\begin{keybox}[title={When Intent Itself Is Unclear or Conflicted}, breakable=false]
	...
	\smallskip
	\textbf{Unexamined tradeoffs}: The user may not have considered tradeoffs they would care about if surfaced, such as speed versus thoroughness, cost versus quality, or brevity versus nuance.
\end{keybox}

When an agent detects these deeper problems, it should surface the conflict instead of silently resolving it: ``You asked for a one-page summary, but covering the indemnification, representations, and covenant issues adequately would demand three to four pages. Would you prefer (a) a one-page overview that flags issues without detailed analysis, (b) a longer memo with full analysis, or (c) detailed treatment of just one area?'' This is harder than detecting ambiguous expression because it needs recognizing when the user's own mental model is incomplete or conflicted, and surfacing that recognition without appearing to second-guess or condescend.
...
\begin{tikzpicture}[
	...
	\node[inbox] (input) at (-2.0, 0) {
		\textcolor{gray-500}{\tiny\sffamily\bfseries USER INSTRUCTION}\\[4pt]
		\small\sffamily\textit{``Review this credit}\\
		\small\sffamily\textit{agreement for risks''}
	};
	...
	% Right: Extracted goal
	\node[goalbox, right=0.8cm of input] (goal) {
		\textcolor{example-dark}{\small\sffamily\bfseries Extracted Goal}\\[3pt]
		\scriptsize\sffamily
		\begin{tabular}{@{}l@{\hspace{0.4em}}l@{}}
			\textcolor{gray-600}{Task:}        & Document review                    \\
			\textcolor{gray-600}{Perspective:} & Lender                             \\
			\textcolor{gray-600}{Focus:}       & Credit risk, collateral, covenants \\
			\textcolor{gray-600}{Deliverable:} & Risk summary memo                  \\
			\textcolor{gray-600}{Success:}     & Material risks identified          \\
		\end{tabular}
	};

\end{tikzpicture}
\caption{Goal extraction for a credit agreement review, showing how ambiguous instructions are refined through context gathering into structured specifications.}
\label{fig:agents2-credit-goal-extraction}
\end{figure}
...
\begin{figure}[htbp]
	...
	\begin{tikzpicture}[scale=0.84, every node/.style={scale=0.84},
		...
		\fill[white, rounded corners=4pt] (-5.5, 0.8) rectangle (4.2, -8.5);
		\draw[gray-300, rounded corners=4pt] (-5.5, 0.8) rectangle (4.2, -8.5);

		% Chat header bar
		\fill[gray-200, rounded corners=4pt] (-5.5, 0.8) rectangle (4.2, 0.2);
		\node[font=\scriptsize\sffamily\bfseries, text=gray-600] at (-0.65, 0.5) {Portfolio Assistant};

		% User message 1 (right aligned)
		\node[userbubble] (u1) at (3.6, -0.3) {
			\parbox{5.5cm}{\raggedleft Rebalance to reduce tech exposure}
		};

		% Agent clarification (left aligned)
		\node[agentbubble] (a1) at (-4.9, -1.8) {
			\parbox{6.5cm}{Current tech exposure is 35\%. What's your target? Should I reduce through sales or reallocation? Any tax considerations?}
		};

		% User response (right aligned)
		\node[userbubble] (u2) at (3.6, -3.8) {
			\parbox{5.5cm}{\raggedleft Get to 25\% through sales. Minimize taxes; use loss harvesting.}
		};

		% Agent validation (left aligned)
		\node[agentbubble] (a2) at (-4.9, -5.4) {
			\parbox{6.5cm}{I'll prepare a plan to reduce tech from 35\% to 25\% via sales, optimized for tax efficiency. I'll need your approval before executing.}
		};

		% Extracted goal box (to the right, showing internal state)
		\node[goalbox, anchor=north west] (goal) at (5.0, -1.2) {
	...
	\end{tikzpicture}
	\caption{Clarification dialogue for a portfolio rebalancing task. The agent detects ambiguity, gathers constraints through targeted questions, and extracts a validated goal before proceeding.}
	\label{fig:agents2-portfolio-clarification}
\end{figure}

\begin{keybox}[title={Intent Understanding Is Continuous}, breakable=false]
	...
	\smallskip
	Effective agents surface these discoveries through clarification instead of proceeding with outdated or impossible goals. Intent understanding is iterative, not instantaneous.
\end{keybox}
