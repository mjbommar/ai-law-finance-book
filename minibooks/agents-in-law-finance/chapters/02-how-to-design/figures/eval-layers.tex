...
\begin{tikzpicture}[
  ...
  desc/.style={font=\small\sffamily},
  mapping/.style={font=\footnotesize\sffamily, text=definition-dark}
]
...
\node[box, fill=bg-key, draw=border-key, line width=1.5pt] (layer3) at (0,4) {
  ...
  {\footnotesize\sffamily GPA+IAT: Goal (G), Iteration (I), Termination (T)} \\[0.1em]
  {\footnotesize\sffamily Testing: Integration testing}
};
...
\node[box, fill=bg-definition!30!bg-key, draw=border-definition, line width=1.5pt] (layer2) at (0,1.5) {
  ...
  {\footnotesize\sffamily GPA+IAT: Adaptation (A)} \\[0.1em]
  {\footnotesize\sffamily Testing: Unit testing}
};
...
\node[box, fill=bg-definition, draw=border-definition, line width=1.5pt] (layer1) at (0,-1) {
  ...
  {\footnotesize\sffamily GPA+IAT: Perception (P)} \\[0.1em]
  {\footnotesize\sffamily Testing: Component testing}
};

% Vertical flow arrows showing progression
\draw[-{Stealth[length=3mm]}, line width=1.5pt, definition-dark]
  (layer1.north) -- (layer2.south) node[midway, right, font=\small\sffamily] {builds on};
\draw[-{Stealth[length=3mm]}, line width=1.5pt, definition-dark]
  (layer2.north) -- (layer3.south) node[midway, right, font=\small\sffamily] {builds on};

% Side annotations for example metrics
\node[anchor=west, font=\footnotesize\sffamily, text width=3.5cm] at (5.5,4) {
  \textbf{Metrics:} \\
  • Task success rate \\
  • Step efficiency \\
  • Resource usage
...
\node[anchor=west, font=\footnotesize\sffamily, text width=3.5cm] at (5.5,1.5) {
  \textbf{Metrics:} \\
  • Strategy quality \\
  • Error recovery \\
  • Adaptation speed
...
\node[anchor=west, font=\footnotesize\sffamily, text width=3.5cm] at (5.5,-1) {
  \textbf{Metrics:} \\
  • Precision@k \\
  • Recall@k \\
  • MRR, nDCG
...
% Foundation label
\node[font=\footnotesize\itshape\sffamily, text=text-muted] at (0,-2.5) {
