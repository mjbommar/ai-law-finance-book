...
To illustrate, a single-shot classifier might have goals, perception, and action capabilities, but lacks the iteration, discretion, and termination properties needed for an agentic system.

\textbf{Property Attribution.} Individual features can be described as ``agentic'' (such as ``agentic perception'' or ``agentic planning'') without implying the system as a whole qualifies as an $\mathit{AgenticSystem}$. Full agentic system status demands all six properties.
\end{theorembox}
...
\begin{practicebox}[title={\textbf{Termination Audit Checklist for Deployments}}]
...
Understanding what makes an entity agentic is only the first step. In regulated domains like law and finance, deploying AI agents calls for robust governance frameworks that ensure compliance, maintain professional standards, and protect sensitive information. The stakes are particularly high: errors can trigger regulatory sanctions, breach fiduciary duties, or compromise client confidentiality.
...
\begin{keybox}[title={\textbf{Six Governance Controls for Professional Deployment}}]
\glsadd{escalation}
\begin{enumerate}[leftmargin=1.5em]
\item \textbf{Attribution and Provenance.} Every factual claim must be traceable to its source---case law, regulatory filings, market data, or expert analysis. Modern AI agents should maintain citation chains that satisfy legal brief or investment memorandum standards.

\item \textbf{Auditable Reasoning.} Log major reasoning steps, tool invocations, and decision points in forms that preserve privilege while facilitating review. Capture the business logic an expert would expect to see: litigation hold decisions, trade execution rationales, compliance determination paths.

\item \textbf{Bounded Operation.} Enforce explicit limits: computational budgets, time constraints, maximum iteration counts. In financial contexts, limit market data queries; in legal contexts, cap document review cycles.

\item \textbf{Escalation Pathways.} Clear triggers---low confidence, high-stakes decisions, detected conflicts---should route matters to human review. A contract review agent might escalate unusual liability provisions or non-standard jurisdiction clauses.

\item \textbf{Confidentiality Controls.} Enforce ethical walls between matters, role-based access controls, and redaction rules. This encompasses legal privilege, insider trading restrictions, and client confidentiality duties.

\item \textbf{Accountability Mapping.} Establish clear responsibility lines before deployment: Who configured the agent? Who reviewed its outputs? Who authorized deployment? These questions must have answers before processing the first client matter.
\end{enumerate}
\end{keybox}
