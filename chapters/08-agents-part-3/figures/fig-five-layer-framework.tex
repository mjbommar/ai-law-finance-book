% Five-Layer Governance Framework — refreshed 3D layer cake (no missing slice)
...
\newcommand{\Layer}[4]{%
    ...
    \draw[label arrow] (\Rx, \yMid) -- ++(\LabelXShift,0)
        node[label text] {\footnotesize\textbf{#3}\\[-2pt]{\scriptsize\textit{#4}}};
}
...
\node[rotate=90, anchor=center] at ({-\Rx-0.6-0.35}, \TotalH/2) {\scriptsize Increasing specificity};
