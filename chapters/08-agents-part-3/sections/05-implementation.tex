...
The system iterates through analysis-recommendation-feedback loops over days or weeks, adapts its strategy based on client responses and market changes, and terminates when confidence thresholds are met or escalation is required. \textit{Dimensional profile: HITL + hybrid frame + adaptive goals + stateful.}

The risk assessment identifies five primary concerns. \emph{Compliance risk} (high likelihood, high impact) arises because the system may recommend unsuitable investments violating Advisers Act fiduciary duty; unlike a simple Q\&A chatbot, iteration and adaptation create compounding risk across multiple cycles. \emph{Accuracy risk} (moderate likelihood, high impact) stems from potential hallucination of market data or misinterpretation of client constraints across iterative cycles, leading to compounding errors. \emph{Adaptation risk} (moderate likelihood, high impact) occurs when the system adapts its strategy in ways that drift from regulatory compliance—for example, learning to recommend higher-fee products based on firm incentives rather than client best interest. \emph{Iteration risk} (low likelihood, high impact) manifests as either excessive iteration (analysis paralysis) or premature termination (incomplete analysis). \emph{Security risk} (low likelihood, very high impact) involves prompt injection across iterative cycles that could manipulate accumulated state, causing disclosure of other clients' information. Table~\ref{tab:agents3-financial-planning-risk} summarizes the risk assessment and key mitigations.

\begin{table}[htbp]
\centering
\caption{Risk Assessment: Agentic Financial Planning Assistant}
\label{tab:agents3-financial-planning-risk}
\footnotesize
\begin{tabular}{@{}>{\sffamily\raggedright\arraybackslash}p{1.8cm} >{\centering\arraybackslash}p{1.4cm} p{0.02cm} >{\centering\arraybackslash}p{1.4cm} >{\raggedright\arraybackslash}p{5.5cm} >{\centering\arraybackslash}p{1.4cm}@{}}
\toprule
\textbf{Risk Category} & \textbf{Likelihood} & & \textbf{Impact} & \textbf{Key Mitigations} & \textbf{Residual Risk} \\
\midrule
Compliance & \cellcolor{red-100}\textcolor{red-900}{High} & & \cellcolor{red-100}\textcolor{red-900}{High} & HITL approval before client recommendations; monthly compliance review; quarterly fiduciary assessment & \cellcolor{amber-100}\textcolor{amber-900}{Moderate} \\
\addlinespace
Accuracy & \cellcolor{amber-100}\textcolor{amber-900}{Moderate} & & \cellcolor{red-100}\textcolor{red-900}{High} & Verified data sources (Bloomberg, Reuters); cross-cycle consistency checks; human review of final recommendations & \cellcolor{amber-100}\textcolor{amber-900}{Moderate} \\
\addlinespace
Adaptation & \cellcolor{amber-100}\textcolor{amber-900}{Moderate} & & \cellcolor{red-100}\textcolor{red-900}{High} & Adaptation limited to analysis methods; recommendation criteria fixed; quarterly adaptation audit & \cellcolor{green-100}\textcolor{green-900}{Low - Moderate} \\
\addlinespace
Iteration & \cellcolor{green-100}\textcolor{green-900}{Low} & & \cellcolor{red-100}\textcolor{red-900}{High} & Explicit termination conditions (5 cycles OR conf.\ >0.85 OR 14-day timeout); human review on timeout & \cellcolor{green-100}\textcolor{green-900}{Low} \\
\addlinespace
Security & \cellcolor{green-100}\textcolor{green-900}{Low} & & \cellcolor{red-100}\textcolor{red-900}{Very High} & Input sanitization per cycle; state integrity validation; client data isolation; monthly penetration testing & \cellcolor{green-100}\textcolor{green-900}{Low} \\
\bottomrule
\end{tabular}
\end{table}
...
The framework proceeds through three phases. \textbf{Phase 1 (Assessment)} uses questionnaires to gather information about the vendor's data sources, model architecture, security practices, performance benchmarks, and fairness testing methodology. \textbf{Phase 2 (Document Review)} examines supporting documentation---SOC 2 reports, data processing agreements, model validation reports, and security certifications---to verify vendor claims. \textbf{Phase 3 (Validation)} confirms claims through reference checks with existing clients in similar domains and pilot testing with representative data to validate accuracy, explainability, and performance before full deployment.

\input{figures/fig-vendor-diligence}
...
\bottomrule
\end{tabular}
\caption{Five categories of adaptation transparency clauses for agentic vendor contracts.}
\label{tab:agents3-adaptation-clauses}
\end{table}

\begin{highlightbox}[title={Example Contractual Language: Adaptation Transparency}, breakable=false]
\small
...
\input{figures/fig-incident-response-cycle}
