...
\begin{keybox}[title={Critical Questions for Evaluating ``Agentic'' Systems}]
	When someone claims a system is ``agentic,'' we should ask:
	\begin{itemize}[nosep]
		\item \textbf{Autonomy:} How much discretion does it have in selecting goals and tactics? Where does accountability reside?
		\item \textbf{Entity:} Is this a pure machine system, a human-AI hybrid, or an institutional arrangement?
		\item \textbf{Goals:} Does it accept goals passively, adapt them through interpretation, or negotiate them with other agents?
		\item \textbf{Persistence:} Does it maintain state and pursue objectives over time, or operate in single-shot mode?
	\end{itemize}
\end{keybox}
