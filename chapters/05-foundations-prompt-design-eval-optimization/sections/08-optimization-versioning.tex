...
\midrule
\textbf{Prompt Templates} & System prompt, instruction templates, variable placeholders & Core logic; changes affect all calls \\
\addlinespace
\textbf{Exemplar Sets} & Few-shot examples with metadata (source, date, label) & Examples shape model behavior; changes affect accuracy \\
\addlinespace
\textbf{Model Configuration} & Model ID, temperature, top-p, max\_tokens, stop sequences & Parameters affect output distribution and cost \\
\addlinespace
\textbf{Input Schemas} & JSON Schema or spec for input validation & Defines valid inputs; prevents malformed requests \\
\addlinespace
\textbf{Output Schemas} & JSON Schema or spec for output validation & Defines expected structure; enables automated checks \\
\addlinespace
\textbf{Validation Rules} & Custom checks (citation fidelity, date ranges, etc.) & Task-specific correctness rules \\
\addlinespace
\textbf{Gold Test Sets} & Test cases with inputs and expected outputs & Regression detection; version with test set version tag \\
\addlinespace
\textbf{Evaluation Scripts} & Code that runs tests and computes metrics & Ensures consistent evaluation across versions \\
\addlinespace
\textbf{Baseline Metrics} & Performance scores from previous versions & Comparison baseline for new versions \\
\bottomrule
\end{tabular}
...
\subsection{Forward References: Scaling to Agentic Systems}
...
\begin{itemize}
  \item \textbf{Multi-component versioning}: Agents compose multiple prompts, tools, and retrieval components. Chapter 7's governance patterns address how to version and test these compositions.
  \item \textbf{Structured logging and auditability}: Agentic systems produce multi-turn traces that require richer logging than single-call systems. Chapter 7 covers trace management and audit requirements.
  \item \textbf{Continuous evaluation in production}: Chapter 7's telemetry and monitoring patterns extend the offline evaluation techniques here to live traffic.
\end{itemize}
