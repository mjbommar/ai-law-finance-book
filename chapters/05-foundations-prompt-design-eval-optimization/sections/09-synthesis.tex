...
\begin{itemize}
  \item \textbf{Agentic architectures} (Chapters~6--8): Multi-step autonomous systems that plan, execute, and adapt. This chapter covers single-call and simple pipeline patterns; agents extend to open-ended task completion.

  \item \textbf{Knowledge graphs} (Chapters~9--10): Structured knowledge representation that complements retrieval-augmented generation. Knowledge graphs enable reasoning over relationships that flat document retrieval cannot capture.
...
\begin{keybox}[title={What Comes Next}]
\begin{itemize}
  \item \textbf{Chapter~6 (Agents Part I)}: Vocabulary and mental models for agentic systems. We define what agents are, distinguish levels of autonomy, and establish the conceptual framework.

  \item \textbf{Chapter~7 (Agents Part II)}: Architecture and protocols for multi-step agents. We cover planning, tool orchestration, memory, and the trace structures that extend the evidence record to agent workflows.

  \item \textbf{Chapter~8 (Agents Part III)}: Governance, deployment, and oversight for agentic systems. We address the unique risks and controls required when AI systems operate with greater autonomy.
\end{itemize}
