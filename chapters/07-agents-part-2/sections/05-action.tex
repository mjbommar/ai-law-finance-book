...
\subsubsection{Communication Tools (Partially Reversible)}

Communication tools send information to others:
...
\subsubsection{Document Management Tools (Largely Reversible)}

Document management tools create and organize work product:
...
\subsubsection{Filing and Submission Tools (Largely Irreversible)}

Filing tools submit documents to external authorities:
...
\subsubsection{Transaction Execution Tools (Irreversible or Costly)}

Transaction tools execute binding transactions:
...
\subsubsection{MCP Tools}

\keyterm{MCP Tools} are executable functions that may change state. Unlike Resources (read-only), Tools can:
...
\subsubsection{MCP Prompts}

\keyterm{MCP Prompts} are reusable templates for common tasks. For action tools, prompts encode standard operating procedures:
...
\subsubsection{Core Controls}

All action tools must implement:
...
\subsubsection{Threat-Specific Mitigations}

\textbf{Prompt injection through action parameters}: Adversaries embed instructions in parameters the agent passes to action tools. \textit{Mitigation}: Sanitize all parameters; never pass raw user input directly to action interfaces; validate parameter formats against strict schemas.
...
\subsubsection{Single Approver}

The agent completes preparation and presents to one designated approver. Appropriate for routine actions with clear approval authority.
...
\subsubsection{Multi-Party Approval}

High-stakes actions require multiple independent approvers. Appropriate for actions with significant financial or legal exposure.
...
\subsubsection{Escalating Approval}

Approval authority escalates with transaction size or risk. Routine actions have lower approval thresholds; exceptional actions escalate to senior personnel.
...
\subsubsection{Approval Request Design}
...
\subsection{Rollback Capabilities}
\label{sec:agents2-rollback}

When actions fail or produce wrong results, rollback capabilities limit damage:

\subsubsection{Fully Reversible Actions}

For actions like document creation or internal system updates:

\textbf{Pattern}: Maintain version history. If the agent creates document version 3 and it is wrong, restore version 2. No external impact.

\subsubsection{Partially Reversible Actions}

For actions like internal communications:

\textbf{Pattern}: Follow-up correction. If the agent sent an incorrect internal alert, send a corrected alert with clear indication that it supersedes the prior message.

\subsubsection{Largely Irreversible Actions}

For actions like court filings:

\textbf{Pattern}: Amendment or correction filing. If the agent filed a document with errors, file an amended version. The original remains visible but is superseded. May require court permission.

\subsubsection{Irreversible Actions}

For actions like trade execution:

\textbf{Pattern}: Offsetting transaction. If the agent executed a wrong trade, execute an offsetting trade to close the position. The offset occurs at current market prices, potentially locking in losses. This is damage mitigation, not reversal.

\textbf{Pattern}: Incident response. Immediately escalate to human control. Stop further automated actions. Begin investigation and remediation.
...
Cap how many actions the agent can take per time period:
...
Automatic stops when anomalies are detected:
...
Chapter~8 examines action governance in detail: actuation controls that limit what agents can do, approval gates that require human verification, and audit trails that enable accountability.
