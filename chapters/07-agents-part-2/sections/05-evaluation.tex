...
\paragraph{Jurisdictional Accuracy} Research shows top legal AI systems achieve only \textbf{75--82\% jurisdictional accuracy}---nearly one in four documents from the wrong jurisdiction.
...
\paragraph{Citation Verification} Research found \emph{incomplete answer rates} range from \textbf{18\% to 62\%} across platforms.
...
OWASP 2025 ranks \emph{prompt injection} as the critical vulnerability, appearing in over \textbf{73\%} of deployments \parencite{owasp-llm-top10}. Attack success rates reach \textbf{84\%} for code-generation agents.
...
\paragraph{VLAIR} First benchmark comparing legal AI against lawyer control groups. Seven tasks including Document Q\&A, Summarization, Redlining. Key finding (October 2025): AI scored \textbf{7 points above lawyer baseline} (\textbf{71\% accuracy}), outperforming on routine tasks but falling short on complex judgment-intensive work.
...
The improvement cycle has eight stages: \textbf{Deploy} with monitoring enabled $\rightarrow$ \textbf{Monitor} metrics and collect feedback $\rightarrow$ \textbf{Sample} outputs for expert review $\rightarrow$ \textbf{Analyze} failures for root causes $\rightarrow$ \textbf{Expand} reference standards with validated failures $\rightarrow$ \textbf{Improve} agent based on analysis $\rightarrow$ \textbf{Validate} against expanded references $\rightarrow$ \textbf{Deploy} improved agent.
