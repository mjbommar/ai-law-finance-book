...
A complete handoff weaves together five elements. The \textbf{situation summary} orients the human quickly: what is the matter, what was the agent trying to accomplish, and why does it matter? The \textbf{progress report} explains what has been done and what remains, so the human understands where in the workflow the escalation occurred. The \textbf{trigger explanation} clarifies why the agent is escalating \textit{now}---what specific condition or finding prompted the handoff. The \textbf{gathered information} presents the relevant findings, even if partial or uncertain; the agent should not withhold work product just because it is incomplete. Finally, the \textbf{recommendation} proposes a path forward with supporting reasoning, giving the human a starting point for their decision rather than a blank slate.
...
Assessing whether escalation mechanisms work correctly requires testing across multiple dimensions, each probing a different failure mode.

\textbf{Coverage} asks whether all situations that \textit{should} trigger escalation actually do. Testing should deliberately construct edge cases: conflicting data that falls just below obvious thresholds, novel situations that differ subtly from training examples, and compound conditions where no single factor would trigger escalation but the combination should. Gaps in coverage mean that situations requiring human judgment will instead receive autonomous action---precisely the failure mode escalation is designed to prevent.

\textbf{Calibration} asks whether thresholds are set correctly. Escalating too aggressively wastes human attention on decisions the agent could handle, eventually training humans to approve without review. Escalating too rarely exposes the organization to risks that should have received oversight. The goal is a calibrated system where escalation signals genuinely require human judgment, maintaining the value of human attention as a scarce resource.

\textbf{Latency} measures whether escalation reaches the human quickly enough to be useful. For time-sensitive matters---a filing deadline approaching, a market moving, a client waiting---escalation that arrives too late is equivalent to no escalation at all. Urgent matters require immediate notification through appropriate channels, not a queue that will be reviewed tomorrow.

\textbf{Routing} ensures escalation reaches the \textit{right} person, not just any person. Legal questions should route to attorneys with relevant expertise; risk limit breaches should reach risk managers with authority to approve exceptions; client-facing decisions should involve relationship managers who understand the context. Misrouted escalation creates delay and frustration while failing to bring appropriate expertise to bear.

\textbf{Context quality} determines whether the human can actually make a decision based on the escalation message alone. If the recipient must investigate from scratch to understand what the agent was doing and why it stopped, the handoff has failed. The escalation should read like a well-prepared briefing, not a cryptic alert requiring interpretation.

\textbf{Response handling} closes the loop by verifying that the agent correctly incorporates human guidance. When the human provides direction---approve this action, reject that recommendation, try a different approach---does the agent actually follow through? Testing should cover the full escalation cycle from trigger through human response through resumed agent action, not just the outbound notification.
