...
\textbf{Emerging surfaces} extend beyond these three primary patterns. \textit{Embedded copilots} integrate agentic capabilities directly into existing applications---word processors, spreadsheets, or domain-specific software---combining familiar graphical interfaces with intent-based interaction. Users remain in their primary workflow while accessing agent capabilities through sidebars or inline suggestions. \textit{Ambient interfaces} use voice or environmental sensors to enable hands-free interaction, though transcription errors and authentication challenges currently limit their use in high-stakes professional contexts. \textit{Agentic APIs} expose capabilities to other software systems rather than human users, enabling machine-to-machine orchestration where the ``user'' is another system---a pattern we revisit in \Cref{sec:agents2-delegation}.

Effective surface design draws on decades of human-computer interaction research, from direct manipulation interfaces to conversational design patterns. A full treatment of interaction design principles belongs to UX practice and extends beyond our scope here. What architects need to understand is that surface selection has architectural consequences: conversational surfaces require low-latency response paths, automation surfaces require robust background processing, and document surfaces require generation pipelines that can produce polished output. The disciplines must collaborate---UX designers choose surfaces based on user needs, and architects build the infrastructure those surfaces require.
