...
\keyterm{Goal}: The desired end state the intent points to (e.g., ``produce a lender-risk memo that meets policy''), as defined in Chapter~6's GPA framework.
...
\subsection{The Challenge of Intent Understanding}
\label{sec:agents2-intent-challenge}
...
Pre-LLM systems handled intent through rigid parsing: keyword matching, slot filling, decision trees. These systems worked for narrow domains with controlled vocabularies but broke on natural language variation. ``Find cases on personal jurisdiction'' and ``What's the law on where you can sue someone?'' express similar intents but look nothing alike to a keyword matcher.

The gap between instruction and intent has always existed; what has changed is our ability to bridge it.

% ----------------------------------------------------------------------------
% How LLMs Transformed Intent Inference
% ----------------------------------------------------------------------------

\subsection{How LLMs Transformed Intent Inference}
\label{sec:agents2-llm-intent}

Large language models dramatically improved the ability to infer intent from natural language. Where rule-based systems required exact matches, LLMs handle variation, implicit context, and domain-specific jargon. As a result, agents can now work with the messy, ambiguous instructions that professionals actually give.

\subsubsection{Capabilities}

Modern LLMs excel at several intent-relevant tasks:

\textbf{Handling noisy input}: Users misspell, use shorthand, include tangential information. LLMs parse through noise to extract meaning. ``Pls check if the SOL has run on that SEC 10b claim from 2019'' is interpretable despite abbreviations and casual language.

\textbf{Resolving references}: When the user says ``that case we discussed'' or ``the Smith deal,'' LLMs can often resolve references using conversational context. This enables multi-turn clarification dialogues.

\textbf{Inferring domain-specific intent}: Legal and financial language carries domain-specific meaning. ``Material'' in a securities context differs from everyday usage. ``Review'' in legal practice implies more than casual reading. LLMs trained on domain text internalize these distinctions.

\textbf{Detecting implicit constraints}: Instructions often omit constraints that professionals take for granted. ``Research the statute of limitations'' implicitly means ``for this jurisdiction, for this claim type, as of now.'' LLMs can infer these implicit constraints from context.

\subsubsection{Remaining Challenges}

Despite dramatic improvement, intent understanding remains imperfect:

\textbf{Long-context degradation}: As conversations extend, LLMs may lose track of earlier context or constraints. An instruction that references ``what we discussed at the beginning'' may not be properly connected.

\textbf{Multi-turn clarification}: When clarification is needed, LLMs sometimes proceed with a default interpretation rather than asking. The result is a ``helpful but wrong'' failure: the agent does \textit{something} reasonable rather than confirming it understood correctly.

\textbf{Implicit meaning}: Professionals communicate through implication. ``This needs to be right'' signals high stakes. ``When you get a chance'' signals low urgency. These signals may not be explicitly parsed.

\textbf{Ambiguity exploitation}: Research has shown that LLMs can ``exploit loopholes'' by selectively misunderstanding ambiguous requests in ways that appear helpful but avoid difficult work. Governance must monitor for this failure mode.
...
The first step classifies the instruction into task types that determine workflow. This translates raw words into candidate \textbf{tasks} that can advance the underlying \textbf{goal}.
...
\subsubsection{Entity and Constraint Recognition}

Beyond classification, the agent must extract entities (what the task concerns) and constraints (what bounds apply):
...
\subsubsection{Structured Goal Representation}

Extracted components can be organized into structured representations that guide execution. This structured representation resembles a short assignment memo that the planning system can act on (\Cref{sec:agents2-planning}) and the termination system can measure against (\Cref{sec:agents2-termination}):
...
\subsubsection{When to Clarify}

The decision to clarify depends on ambiguity severity and action stakes:
...
\subsubsection{How to Clarify}

Effective clarification is specific, contextual, and actionable:
...
Beyond explicit instructions, agents must identify constraints that bound acceptable execution:

\subsubsection{Constraint Types}

\textbf{Temporal constraints}: Deadlines and time windows. Some are explicit (``by Friday''); others are implicit (court filing deadlines calculated from rules); still others are contextual (``before the board meeting'' requires knowing when the meeting is).
...
\subsubsection{Inferring Implicit Constraints}

Professionals operate under constraints they rarely state explicitly. When a partner says ``research Section 10(b) liability,'' implicit constraints include:
...
\subsection{Intent Validation Patterns}
\label{sec:agents2-validation}

Before executing, agents should validate their understanding of intent. Several patterns support validation:

\subsubsection{Reflection and Summarization}

The agent restates its understanding before proceeding:

\begin{quote}
``I understand you want me to review the Smith Credit Agreement, focusing on financial covenants and events of default, and prepare a summary memo identifying provisions that differ from our standard template. I'll complete this by end of day Thursday. Is that correct?''
\end{quote}

This gives the user an opportunity to correct misunderstandings before work begins.

\subsubsection{Chunked Validation}

For complex tasks, validate in phases rather than all at once. After completing research, summarize findings and confirm direction before drafting. After drafting, confirm the approach before finalizing. Each checkpoint prevents error propagation.

\subsubsection{Confidence Signaling}

The agent should also indicate how confident it is in its own understanding. When confidence is high, it is usually appropriate for the agent to proceed with only light oversight, much as you would with a routine assignment given to an experienced associate. When confidence is low, the right move is to pause and ask for clarification rather than press ahead. Clear confidence signaling helps users decide how much review is needed and whether to treat the output as a draft, a starting point, or a near-final product.

% ----------------------------------------------------------------------------
% Legal Example: Credit Agreement Review
% ----------------------------------------------------------------------------

\subsection{Legal Example: Credit Agreement Review}
\label{sec:agents2-intent-legal}

Consider how intent extraction works for a legal research task:

\textbf{Instruction}: ``Review this credit agreement for risks''

\textbf{Step 1 - Intent Classification}: Document review task, not research or drafting.

\textbf{Step 2 - Ambiguity Detection}: ``Risks'' is ambiguous. Risks to whom? What types of risks?

\textbf{Step 3 - Context Gathering}: Check matter context. This is a lender-side engagement for a senior secured facility.

\textbf{Step 4 - Constraint Inference}: Implicit constraints include focus on lender risks (given the engagement), prioritize material issues, assume current market terms as baseline.

\textbf{Step 5 - Clarification}: ``I'll review from the lender's perspective, focusing on credit risk, collateral coverage, and covenant adequacy. Should I also flag documentation risks (drafting issues, missing provisions) or focus only on substantive credit terms?''

\textbf{Step 6 - Goal Extraction}:
\begin{verbatim}
{
  "task_type": "document_review",
  "perspective": "lender",
...
\textbf{Step 7 - Validation}: ``I'll prepare a memo identifying material risks to the lender, comparing key terms to market standards, and recommending negotiation points. I'll have a draft for your review by tomorrow afternoon.''

% ----------------------------------------------------------------------------
% Financial Example: Portfolio Rebalancing
% ----------------------------------------------------------------------------

\subsection{Financial Example: Portfolio Rebalancing}
\label{sec:agents2-intent-financial}

\textbf{Instruction}: ``Rebalance to reduce tech exposure''

\textbf{Step 1 - Intent Classification}: Portfolio action task with analysis component.

\textbf{Step 2 - Ambiguity Detection}: Multiple ambiguities---how much reduction? Through what mechanism? What constraints?

\textbf{Step 3 - Context Gathering}: Pull current portfolio composition. Tech exposure is 35\% against a 25\% target.

\textbf{Step 4 - Clarification Dialogue}:

\textit{Agent}: ``Current tech exposure is 35\%. The target allocation is 25\%. Should I (a) reduce to target (25\%), (b) reduce to a specific level you specify, or (c) reduce by a specific amount?''

\textit{User}: ``Get it to target.''

\textit{Agent}: ``Should I (a) sell tech positions, (b) buy non-tech positions with new cash, (c) use both approaches, or (d) hedge with options?''

\textit{User}: ``Sell, but watch the tax impact.''

\textbf{Step 5 - Constraint Extraction}:
\begin{itemize}[nosep]
\item Target: 25\% tech exposure
\item Mechanism: Sales only
\item Constraint: Minimize tax impact (prefer losses, long-term gains over short-term)
\item Implicit: Stay within risk limits, maintain diversification, execute over reasonable timeframe
\end{itemize}
...
\textbf{Step 7 - Validation}: ``I'll prepare a rebalancing plan to reduce tech from 35\% to 25\% through sales, optimized for tax efficiency. I'll prioritize positions with losses or long-term holding periods. I'll present the plan for your approval before executing any trades.''

% ----------------------------------------------------------------------------
% Integration with Planning and Governance
% ----------------------------------------------------------------------------

\subsection{Integration with Planning and Governance}
\label{sec:agents2-intent-integration}

Intent understanding connects to other framework questions:

\textbf{Memory (Q5)}: Intent extraction improves with context. Prior interactions reveal user preferences; matter history provides case-specific constraints; firm norms establish baseline expectations. Memory enables intent understanding to improve over time.

\textbf{Planning (Q6)}: Extracted goals feed the planning system. Clear intent enables effective decomposition; ambiguous intent propagates through the plan as uncertainty. Poor intent extraction wastes planning effort.

\textbf{Governance (Q10)}: Intent misalignment is a governance risk. If the agent pursues goals the user didn't intend, outputs may be useless or harmful. Chapter~8 addresses purpose limitation controls that verify goal alignment before deployment and monitor for drift during operation.
...
\subsection{Summary}
\label{sec:agents2-intent-summary}

Understanding intent bridges the gap between what users \textbf{say} (instruction) and what they \textbf{mean} (intent), and it shapes the \textbf{goal} and \textbf{tasks} the agent will plan.

Key principles:

\begin{enumerate}[nosep]
\item \textbf{Intent differs from instruction}: The surface request may not capture the underlying goal, constraints, or success criteria.
\item \textbf{Clarification beats guessing}: When ambiguity is significant and stakes are high, ask rather than assume.
\item \textbf{Constraints matter as much as goals}: Time, scope, audience, compliance, and budget bound acceptable outputs.
\item \textbf{Validation prevents wasted effort}: Confirm understanding before significant work.
\end{enumerate}
