...
\textbf{Document Processing Tools}: Raw documents need processing before they are useful:
...
\textbf{Computation Tools}: Some forms of perception require calculation rather than simple lookup:
...
\textbf{MCP Architecture}: The architecture has three roles:
...
\textbf{MCP Resources}: For perception, MCP defines \keyterm{Resources}---read-only data access endpoints. Resources let agents:
...
\textbf{Audit trails}: Every perception must be logged---what was accessed, by which agent, for which matter. This enables compliance review and breach detection. See \Cref{sec:agents2-memory} for detailed treatment of isolation requirements in memory systems.
...
\textbf{Least Privilege}: Perception tools should request only the permissions they need. A legal research tool needs read access to case databases, not write access to the document management system. If a compromised agent gains perception credentials, damage is limited to what those credentials allow.

\textbf{Rate Limiting}: Agents can get stuck in perception loops---searching repeatedly without progress. Tools should track invocation frequency and refuse requests beyond reasonable thresholds. If the agent has searched five times with no results, it should stop and escalate instead of looping indefinitely.
...
When evaluating agentic systems, assess perception against these criteria:
...
\textbf{Action tools change state}. They file documents, send emails, and execute trades. Once executed, some actions cannot be undone. The risks are different, and the governance must be different as well. \Cref{sec:agents2-action} examines action capabilities in detail.
