...
\begin{keybox}[title={Why Agentic Architecture Works}]
	...
	\vspace{0.5em}
	This is not merely automation; it is an architectural solution to a fundamental constraint. Planning (\Cref{sec:agents2-planning}) breaks complex tasks into subtasks. Retrieval fetches specific information for each subtask. The LLM operates within its effective range on each step, even when the overall task would be impossible to handle monolithically.
...
\paragraph{Embeddings and Semantic Similarity}

To retrieve relevant information from large collections, we need a way to measure what ``relevant'' means. \keyterm{Embeddings} provide this capability.

\begin{definitionbox}[title={Embeddings}, breakable=false]
	An \keyterm{embedding} is a numerical representation of content as a vector---a list of numbers that encodes semantic meaning. Similar concepts produce similar vectors; different concepts produce distant vectors. The sentence ``breach of fiduciary duty'' and ``violation of trust obligations'' should have embeddings close together in vector space, even though they share no words.

	\vspace{0.5em}
	Similarity between embeddings is typically measured using \keyterm{cosine similarity} (the angle between vectors) or \keyterm{Euclidean distance} (the straight-line distance between points). These mathematical operations are fast, enabling search across millions of documents in milliseconds.
\end{definitionbox}

Embedding models vary significantly in quality, size, and design goals. Some optimize for speed and compactness; others prioritize semantic precision at higher computational cost. Domain-specific models trained on legal or financial text may outperform general-purpose models for professional applications. The choice of embedding model affects retrieval quality, latency, and infrastructure costs---tradeoffs we discuss in detail in our treatment of embeddings and retrieval techniques elsewhere in this book.

While we focus on text embeddings here, the same principles apply to other content types. Images, audio, and video can all be encoded as embeddings, enabling similarity search across modalities. A scanned contract can be embedded alongside its text; a recorded deposition can be searched by semantic content rather than transcript keywords. Some embedding models are explicitly multimodal, encoding text and images into the same vector space so that a text query can retrieve relevant images and vice versa. The retrieval infrastructure---vector stores and similarity search---works identically regardless of what the embeddings represent.

For text, embeddings transform the problem of finding relevant documents from keyword matching to meaning matching. A search for ``material adverse change'' can find documents discussing ``MAC clauses'' or ``significant negative developments'' because their embeddings are close, even without shared vocabulary.

\paragraph{Vector Stores}

Embeddings enable semantic search, but storing and searching millions of embeddings efficiently requires specialized infrastructure.
...
With these foundations in place, we can now define RAG precisely.
...
\begin{definitionbox}[title={Retrieval-Augmented Generation (RAG)}, breakable=false]
	...
	\begin{enumerate}[nosep]
		\item \textbf{Retrieve}: Given a query, find relevant documents using semantic search (embeddings + vector store) or keyword search.
	...
	\vspace{0.5em}
	RAG works because of in-context learning. The retrieved documents appear in the prompt, and the model adapts its response to incorporate that specific information. RAG is a pattern for \textit{using} retrieval infrastructure, not the infrastructure itself.
\end{definitionbox}

The distinction matters. When evaluating a system, you should ask: What retrieval infrastructure does it use? (Vector store, keyword index, hybrid?) What RAG pattern does it implement? (Basic retrieval, multi-step, with reranking?) These are separate architectural decisions with different tradeoffs.

\begin{keybox}[title={Why Precision Matters}, breakable=false]
	Conflating RAG with vector stores or treating RAG as a ``system'' obscures important design choices. RAG is a prompt pattern that can use various retrieval backends. A RAG implementation might use:
	\begin{itemize}[nosep]
		\item Pure semantic search (embeddings + vector store)
		\item Pure keyword search (BM25, traditional search engines)
		\item Hybrid search (combining both)
		\item Multi-stage retrieval (coarse retrieval followed by reranking)
	\end{itemize}
	Each choice has implications for precision, recall, latency, and cost. Understanding RAG as a pattern rather than a monolithic system helps you evaluate these tradeoffs.
\end{keybox}
