...
Evaluating memory systems for professional practice requires metrics that go beyond generic information retrieval benchmarks. The stakes are too high for abstract measurements; testing must reflect the actual demands of legal and financial work.

Retrieval quality remains foundational, but measurement must be grounded in professional standards. The classic metrics of \textit{precision} (what fraction of retrieved documents are actually relevant?) and \textit{recall} (what fraction of relevant documents did we successfully retrieve?) provide a starting point. For legal applications, the gold standard test is comparison against expert work product: given a research question, does the agent find the same authorities that a skilled attorney cited in their research memo? Missing a controlling case is not merely a statistical shortfall---it could mean missing the argument that wins or loses the matter.

Isolation integrity demands its own testing regime. The goal is simple to state: queries executed in the context of Matter A should never return documents from Matter B. Verification requires adversarial testing that deliberately attempts to breach these walls, probing for edge cases where namespace boundaries might leak. Red-team exercises should try cross-matter queries, test whether embedding similarities can surface isolated documents, and verify that access controls hold even when users attempt to escalate privileges.

Temporal validity measures how well the system maintains currency. In legal contexts, this means tracking how often retrieval surfaces precedent that has been overruled, limited, or superseded---and how quickly new developments propagate into the knowledge base after a court issues a significant opinion or a regulator announces a rule change. Financial applications face even tighter requirements: stale pricing data can render analysis worthless or worse, and the lag between a market event and its reflection in the knowledge base may need to be measured in seconds rather than days.

Finally, scale performance ensures that the system remains responsive as data volumes grow. Episodic memory expands rapidly in professional practice---a single complex litigation matter can generate tens of thousands of documents, and a busy practice accumulates matters over years. Retrieval latency must remain low enough for interactive use even as the underlying corpus grows, which requires careful attention to indexing strategies, hardware provisioning, and architectural choices that allow the system to scale horizontally.
