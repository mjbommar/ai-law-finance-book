% ============================================================================
% 11-governance.tex
% Q10: How Do We Build Systems That Can Be Governed?
% Part of: Chapter 07 - Agents Part II: How to Build an Agent
% ============================================================================

\section{How Do We Build Systems That Can Be Governed?}
...
\textbf{Action logs} record what the agent did (\Cref{sec:agents2-action}). Every action must be logged with the agent identity, timestamp, parameters, and matter/client context. Action logs enable reconstruction: given a complaint about an erroneous filing, you can trace back through the action log to find when it was filed, what parameters were used, and what reasoning preceded it.
...
\textbf{Delegation logs} capture agent-to-agent interactions (\Cref{sec:agents2-delegation}). When a coordinator delegates to specialists, the log must record the delegation contract, the specialist's identity, and the returned artifacts. Complete delegation logging enables audit trails that span the entire delegation tree.

\begin{highlightbox}[title={Structured Logging Schema}]
...
\textbf{Resource-level permissions} control what data an agent can access (\Cref{sec:agents2-mcp-perception}). MCP Resources are explicitly read-only, separating perception from action. An agent with resource access can retrieve documents but cannot modify or file them. This separation enables fine-grained access control that mirrors how organizations already think about permissions.
...
\begin{highlightbox}[title={Privilege Escalation via Tool Chaining}]
...
\toprule
\textbf{Capability} & \textbf{Architectural Requirement} & \textbf{Chapter Cross-References} \\
\midrule
Logging & Structured event capture across all components & Triggers, Intent, Perception, Action, Memory, Planning, Delegation \\
\addlinespace
Override & External control plane with pause/abort/redirect & Action (\Cref{sec:agents2-circuit-breakers}), Termination (\Cref{sec:agents2-loop-detection}) \\
\addlinespace
State Management & Periodic snapshots, checkpoint serialization, rollback support & Action (\Cref{sec:agents2-reversibility}), Termination (\Cref{sec:agents2-graceful-degradation}) \\
\addlinespace
Least Privilege & RBAC at tool/resource/namespace levels & Action (\Cref{sec:agents2-action-security}), Perception (\Cref{sec:agents2-mcp-perception}), Memory (\Cref{sec:agents2-memory-isolation}) \\
\addlinespace
Escalation Hooks & Configurable triggers with reliable routing & Escalation (\Cref{sec:agents2-escalation}), Planning (\Cref{sec:agents2-budgets}) \\
\bottomrule
\end{tabular}
\end{table}
