...
Agent systems require the same infrastructure. Governance is not a single question but a lens through which all other questions must be viewed. Every capability creates governance requirements. Every architectural choice enables or constrains oversight. This section previews governance across all ten questions; Chapter~8 provides comprehensive treatment.

\begin{definitionbox}[title={Agent Governance}]
\keyterm{Agent governance} encompasses the policies, controls, and oversight mechanisms that ensure agents operate safely, ethically, and in compliance with applicable requirements. Governance spans the agent lifecycle: design, deployment, operation, and retirement.
...
You cannot audit what you did not log. You cannot enforce privilege boundaries that were never implemented. You cannot demonstrate bounded operation without termination mechanisms. You cannot explain agent decisions without transparency architecture.

When a regulator asks how the compliance agent detected a breach, when opposing counsel demands production of the agent's reasoning, when a client questions why the agent recommended a particular strategy---architecture determines whether you can answer.

\textbf{Professional duties are non-delegable}: Attorneys remain liable for AI-assisted work product. Fiduciaries remain accountable for AI-informed recommendations. Chapter~8 details those obligations. This chapter provides the architecture to meet them.
\end{keybox}
...
Chapter~8 provides detailed retention frameworks for legal and financial contexts.
...
This chapter answered \textit{how to build an agent}. The ten questions provide architectural foundations:

\begin{enumerate}[nosep]
\item Triggers bring work into the system
\item Intent extraction understands what's being asked
\item Perception tools gather information
\item Action tools effect change
\item Memory maintains context
\item Planning decomposes complex tasks
\item Termination recognizes completion
\item Escalation knows when to ask for help
\item Delegation coordinates with specialists
\item Governance ensures safe operation
\end{enumerate}

Chapter~8 answers the next question: \textit{how do we govern these systems?} It examines:

\begin{itemize}[nosep]
\item The five-layer governance stack (legal, model, system, process, culture)
\item Dimensional controls (autonomy, persistence, goal dynamics)
\item Implementation patterns for each governance requirement
\item Accountability structures and liability allocation
\item Regulatory compliance frameworks
\item Worked examples in legal, financial, and audit contexts
\end{itemize}
...
\vspace{1em}
\noindent\textit{You cannot govern what you do not understand.} This chapter provided the understanding. Chapter~8 provides the governance.
