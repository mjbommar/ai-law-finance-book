...
\begin{definitionbox}[unbreakable,title={\textbf{Operational Definition: Agentic System (GPA+IAT)}}]
	...
	\textit{Total: 6 operational properties (Goal, Perception, Action + Iteration, Adaptation, Termination)}
\end{definitionbox}
...
\textbf{Adaptation (A):} Strategy modification based on accumulated observations and feedback within a session or task.\footnote{This definition focuses on \textit{session-level adaptation}---modifying behavior within a single task execution based on immediate feedback. This differs from \textit{cross-session learning}, where a system improves through model retraining across multiple tasks (e.g., fine-tuning an LLM on user feedback). Both forms of learning are valuable; our definition emphasizes within-session adaptation as the operational property distinguishing agentic systems from static tools. When procurement asks ``does it learn?'' clarify which type: session-level (runtime adaptation) or cross-session (model updates).} The entity adjusts its approach when initial attempts fail, learns which actions succeed, updates its policy based on results.
