...
\begin{highlightbox}[importance=low,title={\textbf{Optional Section: Skip if Time-Pressed}}]
\textbf{You now have working definitions and practical evaluation tools.} This section provides optional historical context—tracing how agency evolved from 1950s philosophy through 1990s distributed systems to 2020s LLM agents.

\textbf{Skip to Section~\ref{sec:furtherlearning} if:}
\begin{itemize}[nosep,leftmargin=1.5em]
\item You need immediate application guidance (use the 6-question rubric from Section~\ref{sec:practical})
\item You're satisfied with the three-level hierarchy and six properties
\item You don't need to understand \textit{why} definitions vary across disciplines
\end{itemize}

\textbf{Read this section if:}
\begin{itemize}[nosep,leftmargin=1.5em]
\item You're writing research or policy requiring theoretical grounding
\item You need to understand disciplinary differences (law vs. economics vs. AI)
\item You want to trace how each property emerged historically
\end{itemize}
\end{highlightbox}
...
\begin{definitionbox}[importance=low,title={\textbf{2013--2015: Deep Reinforcement Learning}}]
